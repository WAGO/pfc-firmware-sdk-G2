...
using netconf::api::MockBridgeConfig;
...
TEST(DSAModeHandlerTest, GetAConfig)
{
  ...
  EXPECT_CALL(mbc, GetBridgeConfig(_)).WillOnce(DoAll(SetArgReferee<0>(returned_config), Return(netconf::Status{})));
...
TEST(DSAModeHandlerTest, SetDSAModeNoChange)
{
  ...
  EXPECT_CALL(mbc, SetBridgeConfig(_)).WillOnce(DoAll(SaveArg<0>(&set_parameter), Return(netconf::Status{})));

  dut.Execute();

  EXPECT_EQ(ToString(expected_config), ToString(set_parameter));
}

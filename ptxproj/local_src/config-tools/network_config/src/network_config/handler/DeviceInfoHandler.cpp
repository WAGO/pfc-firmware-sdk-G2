...
static void KeepInfosOfName(InterfaceInformations &infos, const ::std::optional<::std::string> &keep_interface) {
  if (keep_interface) {
    auto not_mathing_name_or_label = [name=*keep_interface](const InterfaceInformation &info) {
      return !(name == info.GetInterface().GetName());
    };
...
void DeviceInfoHandler::Execute() {
  ...
  ::std::optional<::netconf::DeviceTypes> type_filter_opt;

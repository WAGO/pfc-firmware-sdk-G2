...
void BridgeConfigHandler::GetConfig() {
  ...
  if (Contains(vm_, "device")) {
    ::std::string device {GetDevice(vm_)};
    auto bridge_interfaces = bridge_config.GetBridgeInterfaces(device);
    ::std::vector<::std::string> labels;
    ::std::transform(bridge_interfaces.begin(), bridge_interfaces.end(), ::std::back_inserter(labels), [](auto& interface){return ::netconf::Interface::LabelFromName(interface);});
    ::std::cout << boost::algorithm::join(labels, ",");
  } else if (format == "text") {

...
static void ExpectValue(const po::variables_map &map, const ::std::string &option) {
  if (map.count(option) == 0) {
    return;
  }
  auto value = map[option].as<::std::string>();
  if (value.empty()) {
    throw po::error("Missing value for option '" + option + "'");
  }
}
...
po::options_description OptionParser::CreateDescriptions() const {
  ...
  options.add_options()
    ...
    (options_.interface.name, options_.interface.description)
...
void OptionParser::Parse(int argc, const char **argv) {
  ...
  ExpectOptionPair(map_, options_.interface, options_.add, options_.del);

//------------------------------------------------------------------------------
/// Copyright (c) 2014-2024 WAGO GmbH & Co. KG
///
...
namespace {
  ...
  constexpr int32_t FH_MAX_FILESIZE = 1024 * 1024 * 1024;  // 1GByte maximum file size.
  static_assert(FH_MAX_FILESIZE < UINT32_MAX, "FH_MAX_FILESIZE + 1 has to be less than or equal to uint32_t max.");

  using plainfile_t = struct plainfile {
    int           fd{-1};
    uint32_t      filesize{0};
    struct stat   stat;
    ::std::string filepath;
  ...
  plainfile_t fh_open(const ::std::string &path, const FileAccessMode &mode) {
    ...
    if (file.fd >= 0 && oflags == O_RDONLY) {
      ...
      if (rc == 0) {
        if (file.stat.st_size <= FH_MAX_FILESIZE) {
          file.filesize = static_cast<uint32_t>(file.stat.st_size);
        } else {
          set_error(SYSTEM_CALL_ERROR, 0, "Maximum file size exceeded on " + path);
          close(file.fd);
        }
      } else {
  ...
  size_t fh_read(plainfile_t &pfp, ::std::string &data) {
    ssize_t rc;
    char* buffer = static_cast<char*>( g_malloc0(pfp.filesize + 1));
    ...
    if (rc < 0) {
      ...
      data = "";
      rc = 0;
    } else if (static_cast<size_t>(rc) > FH_MAX_FILESIZE) {
      set_error(FILE_READ_ERROR, errno, "Maximum file size exceeded on " + pfp.filepath);
      data = "";
      rc = FH_MAX_FILESIZE + 1;
    } else {
      data = buffer;
    }
  ...
  ssize_t fh_write(plainfile_t &pfp, const void *data, size_t size) {
    if (size <= 0) {
      return 0;
    }

    ssize_t sz = write(pfp.fd, data, size);
    if (sz != static_cast<ssize_t>(size)) {
      if (errno == ENOSPC) {
  ...
  int fh_lock(plainfile_t &pfp, int operation) {
    ...
    if (rc != 0) {
      set_error(errno, "Failed to acquire/free lock for " + pfp.filepath);
    }
    return rc;
  }
...
using xml_param_t = struct xml_param {
  ...
  xml_param(::std::string pname, ::std::string pjname, service_t pservice, bool pext, bool pmulti,
      ...
      pjname_ {std::move( pjname )},
      pservice_ { pservice },
      pext_ {pext },
      pmulti_ { pmulti },
      pxmlname_ {std::move( pxmlname )},
  ...
  ::std::list<xml_param_t> xml_param_name_list = { // NOLINT(cert-err58-cpp)
  ...
  void xml_check_dnshost(const ::std::string &value) {
    ...
    for (guint i = 0; i < entries.size(); i++) {
      ...
      status = xml_check_condition(is_valid_ip_string(ip_hosts[0]), status);
      if (!status) {
      ...
      if (status) {
        for (auto &name : hostlist) {
          status = xml_check_condition(conv_is_valid_domain_name(name), status);
          if (!status) {
  ...
  void xml_check_dhcpdstate(const ::std::string &value) {
    erh_assert((value == "enabled" || value == "disabled"), INVALID_PARAMETER, "Invalid DHCPD state.");
  }
  ...
  void xml_check_dhcpdrange(const ::std::string& value) {
    ...
    result = xml_check_condition(is_valid_ip_string(ip_addrs[0]), result);
    result = xml_check_condition(is_valid_ip_string(ip_addrs[1]), result);
  ...
  void xml_check_dhcpdlease(const ::std::string &value) {
    ...
    bool result = !value.empty() && std::find_if(value.begin(),
              value.end(), [](unsigned char c) { return std::isdigit(c) == 0; }) == value.end();
  ...
  void xml_check_dhcpdhost(const ::std::string &value) {
    ...
    for(auto& entry : entries) {
      ...
      if (result) {
        result = xml_check_condition(is_valid_ip_string(mac_ip[1]), result);
      }
  ...
  xml_param_t xml_find_param(const ::std::string &parname, service_t serv, bool extonly) {
    auto it = ::std::find_if(xml_param_name_list.begin(), xml_param_name_list.end(),
      [&](xml_param_t &p) {
  ...
  dnsmasq_handle_t ct_dnsmasq_start_session(::std::string &xml_file) {
    ...
    if (SUCCESS != (status = ct_xml_start_session(xml_file.c_str(), &(handle.xml_session)))) {
      erh_set_error(static_cast<eStatusCode>(status), "XML config file missing or wrong format");
    }
    return handle;
  }
  ...
  void ct_dnsmasq_save_xml_file(dnsmasq_handle_t &handle) {
    if (handle.modified) {
      ...
      erh_assert(SUCCESS == status, static_cast<eStatusCode>(status), "Error on writing XML file to disk.");
    }
    handle.modified = false;
  }
  ...
  ::std::string ct_dnsmasq_get_value(
    ...
    if (SUCCESS != status) {
      erh_set_error(static_cast<eStatusCode>(status),
       ::std::string{"XML config file error. Missing config parameter "} + parname, true);
    }
  ...
  void ct_dnsmasq_set_value(dnsmasq_handle_t &handle, service_t service, const ::std::string &port,
    ...
    erh_assert(SUCCESS == status, static_cast<eStatusCode>(status), "Cannot write to XML data.");
  }
  ...
  void ct_dnsmasq_adjust_net(dnsmasq_handle_t &xmlhandle, port_data &pdt) {
    ...
    auto status = conv_ip_ascii2bin(newnetmask, pdt.netmask_);
    ...
    status = conv_ip_ascii2bin(oldnetmask, netmask_str);
    ...
    status = conv_ip_ascii2bin(interface_net, pdt.ip_addr_);
    ...
    auto range = dhcp_range::dhcp_range::fromRangeString(range_str, oldnetmask);
    ...
    if (interface_net != range.getLowerNetwork()) {
      ...
      if(!hosts_str.empty() ) {
        ::std::vector<dhcpd_host_t> ethers;
        status = conv_hosts_ascii2bin(hosts_str, oldnetmask, ethers);
        ...
        conv_hosts_bin2ascii(value, ethers);
  ...
  void ct_dnsmasq_check_ip_dependencies(
      ip_configuration &data, dnsmasq_handle_t &xmlhandle, const std::vector<std::string> &legal_ports) {
   ...
   */
  void ct_dnsmasq_check_dhcp_range(const std::string &rangestr, ip_configuration &ip_config, const std::string &port) {
    ...
    auto range = dhcp_range::dhcp_range::fromRangeString(rangestr, netmask);
  ...
  void ct_dnsmasq_check_dhcp_hosts(const std::string &hosts_string) {
    ...
    auto status = conv_hosts_ascii2bin(hosts_string, dummy_netmask, hosts);
  ...
  void ct_dnsmasq_check_dhcp_parameter(
      const std::string &parameter, const std::string &value, const std::string &port, ip_configuration &ip_config) {
  ...
  DnsmasqParameter ct_dnsmasq_parse_option(
      const std::string& option, service_t service, const std::string &port, ip_configuration &ip_config) {
  ...
  const ::std::string etc_host_marker = "# CONFIG_TOOL_SPECIFIC\n"; // NOLINT(cert-err58-cpp)
  const ::std::string etc_host_marker_description =                 // NOLINT(cert-err58-cpp)
  ...
  const size_t etc_host_marker_size = 2*etc_host_marker.size() + etc_host_marker_description.size();
...
//    erh_assert(status == SUCCESS, status, "Failed to read file %s.", dnsmasq_conf);
  ...
  bool port_is_up(const ::std::string &port,ip_configuration &ip_config_data) {
    for(auto& entry : ip_config_data.port_data_list) {
      if(entry.port_name_ == port) {
        return entry.state_ == "enabled" && entry.link_state_ == netconf::LinkState::UP;
      }
    }
    return false;
  }
  ...
  void append_dhcpd_port_config(std::stringstream &stream,
      dnsmasq_handle_t &handle, const std::string &port, ip_configuration &ip_config) {
   ...
   * @param ip_config
   */
  void ct_dnsmasq_generate_conf( dnsmasq_handle_t &handle, const std::string& dnsmasq_conf_tmp, 
                                 const std::vector<std::string> legal_ports, ip_configuration &ip_config) {
    ...
    if (dnsstate == "enabled") {
      bool foundAtLeastOnePort = false;

      for (auto &port : ip_config.port_data_list) {
        if (port.state_ == "enabled" && port.has_valid_ip_config() && port.link_state_ == netconf::LinkState::UP) {
            stream << "interface=" << port.port_name_ << "\n";
            foundAtLeastOnePort = true;
        }
      }

      if(! foundAtLeastOnePort) {
        stream << "port=0\n";
      }
    } else {
      stream << "port=0\n";
  ...
  void restart_dnsmasqd(const ::std::string &restart_cmd) {
    auto p = program::execute(restart_cmd);
    int ecode = p.get_result();
  ...
  char const *usage_text =
      ...
      "  -T/--interface-statuses <json>  Interface statuses as json string\n"
  ...
  {
      {
        {"services-config", required_argument, nullptr, 'C'} ,
        { "dnsmasq-config", required_argument, nullptr, 'o' },
        { "help", no_argument, nullptr, 'h' },
        { "port", required_argument, nullptr, 'p' },
        { "dns", no_argument, nullptr, 'n' },
        { "dhcp", no_argument, nullptr, 'd' },
        { "get-config", no_argument, nullptr, 'g' },
        { "set-config", no_argument, nullptr, 's' },
        { "gen-config", no_argument, nullptr, 'c' },
        { "dbg-root", required_argument, nullptr, 'r' },
        { "json", no_argument, nullptr, 'j' },
        { "no-dnsmasq-restart", no_argument, nullptr, 'x' },
        { "bridge-config", required_argument, nullptr, 'B' },
        { "ip-config", required_argument, nullptr, 'I' },
        { "interface-config", required_argument, nullptr, 'P' },
        { "interface-statuses", required_argument, nullptr, 'T' },
        { "restore-config", required_argument, nullptr, 'R'},
        { "store-config", required_argument, nullptr, 'S'},
        { nullptr, 0, nullptr, 0 }
      }
  };
  ...
  ::std::string json_get_lease_file(uint32_t network, uint32_t netmask, const std::string& lease_file) {
    ...
    for (auto &line : lines) {
      ...
      auto status_ip_conv = conv_ip_ascii2bin(lease_net, fields[2]);
      ...
      auto lt = static_cast<time_t>(strtoul(fields[0].c_str(), nullptr, 10));
      if (lt >= time(nullptr)) {
        timestr[0] = '\0';
        auto timestruct = localtime(&lt);
        strftime(timestr.data(), sizeof(timestr), "%F %T", timestruct);
        result << delim << R"({"ipAddr": ")" << fields[2] << R"(", "macAddr": ")" << fields[1] << "\", ";
        result << R"("hostName": ")" << fields[3] << R"(", "endOfLease": ")" << timestr.data() << "\"}";
        delim = ", ";
      }
    }
    result << "]";
  ...
  void json_print_dhcpd_conf(dnsmasq_handle_t &handle, const port_data &pd, const std::string &lease_file) {

    ::std::string delim{};
    printf("\"%s\": {", pd.port_name_.c_str());

    for (auto &parameter : xml_param_name_list) {
      if (parameter.pservice_ == DHCPD && parameter.pext_) {
        ...
        if (parameter.pname_ == "dhcpd-fix-host") {
          ...
          if(!value.empty()) {
            ...
            for (auto &r : result) {
              ...
              printf(R"(%s{"hostMac": "%s", "ipAddr": "%s"})", subdelim.c_str(), mac_ip_group[0].c_str(),
          ...
          printf(R"(%s"%s": "%s")", delim.c_str(), parameter.pjname_.c_str(), value.c_str());
        }
        delim = ", ";
      }
    }
  ...
  void json_print_dns_conf(dnsmasq_handle_t &handle) {
    ...
    std::copy_if(xml_param_name_list.begin(), xml_param_name_list.end(),
                 ...
                 [](auto &p) { return p.pservice_ == DNS && p.pext_; });
      ...
      if (parameter.pname_ == "dns-fix-host") {
        ...
        if (!val.empty()) {
          ...
          for (auto &r : result) {
            ...
            printf(R"(%s{"ipAddr": "%s", "names": [)", subdelim.c_str(), ip_name_group[0].c_str());
        ...
        printf(R"(%s"%s": "%s")", delim.c_str(), parameter.pjname_.c_str(), val.c_str());
      }
      delim = ", ";
    }
    printf("}\n");
  ...
  {
    const ::std::map<::std::string,::std::string> port_mapping = {{"br0","X1"},{"br1","X2"},{"br2","X11"},{"br3","X12"}};

    auto it = port_mapping.find(bridge);
    if (it != port_mapping.end())
    {
      return port_mapping.at(bridge);
    }
    return bridge;
  }
  ...
  // Returns optind.
  uint8_t eval_options(int argc, char **argv, prgconf_t &prgconf)
  {
      int c;
      ...
      while(-1 != (c = getopt_long(argc, argv, "C:o:p:hndgsr:cjxB:I:P:T:S:R:", long_options.data(), &option_index)))
      {
          ...
          case 'P':
              ...
              break;
          case 'T':
              prgconf.interface_statuses = optarg;
              break;
          case 'h':
          ...
          default:
              if(argc > 0 && optind > 0) {
                ...
                ::std::string option{gsl::at(args, optind - 1)};
      ...
      erh_assert(optind < UINT8_MAX, INVALID_PARAMETER, "Too many parameters given.");
      return static_cast<uint8_t>(optind);
  }


void print_config(dnsmasq_handle_t &session, const service_t service, const std::string &port,
                  const gsl::span<char *>& args, uint8_t loptind) {
  ...
  if (args.size() > loptind) {
    // Further command line arguments available. Get configuration parameters.
    for (uint8_t idx = loptind; idx < args.size(); idx++) {
      std::string key = std::string { gsl::at(args,static_cast<gsl::index>(idx)) };
      std::string value;

      if (key == "dhcpd-fix-host" || key == "dns-fix-host") {
        value = ct_dnsmasq_get_value(session, service, port, key, "");
      } else {
        value = ct_dnsmasq_get_value(session, service, port, key);
      }
    ...
    for (auto &elem : xml_param_name_list) {
      if (elem.pservice_ == service && elem.pext_) {
...
void print_json_config(
    ...
    ip_configuration &ip_config, const std::string &lease_file) {
...
void set_config(const service_t &service, const std::string &port, const gsl::span<char *>& args, uint8_t loptind,
                const prgconf_t &prgconf, dnsmasq_handle_t &session, ip_configuration &ip_config,
  ...
  for (uint8_t idx = loptind; idx < args.size(); idx++) {
    auto param = ct_dnsmasq_parse_option(gsl::at(args, static_cast<gsl::index>(idx)), service, port, ip_config);
 ...
 */
void generate_config_file(const prgconf_t &prgconf, dnsmasq_handle_t &session, ip_configuration &ip_config,
                          const std::vector<::std::string> &ports) {
...
void restore_config(const prgconf_t &prgconf, dnsmasq_handle_t &session, ip_configuration &ip_config,
  ...
  if(!read_backup_parameters_successfully) {
    ...
    for (const auto& port : legacy_ports) {
      for (auto &parameter : xml_param_name_list) {
        if (parameter.pservice_ == DHCPD && parameter.pext_) {
...
int run(int argc, char **argv) {
  ...
  ip_configuration ip_config_data;
  ...
  erh_init(basename(gsl::at(args, 0)));
  uint8_t loptind = eval_options(argc, argv, prgconf);
  ...
  if (GET_CONFIG == prgconf.command) {
    print_config(dnsmasq_session, serv, port, args, loptind);
    ...
    set_config(serv, port, args, loptind, prgconf, dnsmasq_session, ip_config_data, legal_bridge_names);

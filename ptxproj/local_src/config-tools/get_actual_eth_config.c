...
static int PrintParameter(char *pPortString, char *pParamString);
...
static int NetValueGet(const char *pPortString, uint32_t *value, enum netvalue type)
{
    ...
    {
    case NETMASK:
        status = ct_libnet_get_netmask_from_hw(pPortString, buffer, sizeof (buffer), NETMASK_FORMAT_MODE_CLASSIC, NULL);
        if (SUCCESS == status)
        {
        ...
        break;
    case IPADDR:
        status = ct_libnet_get_ip_addr_from_hw(pPortString, buffer, sizeof (buffer), IP_FORMAT_MODE_NO_NETMASK, NULL);
        if (SUCCESS == status)
        {
            status = ctlib_IpaddrToInt((const char *)buffer, value);
        }
...
static int PrintDefaultGateway(const char *pPortString)
{
    int status = SUCCESS;
    ...
    if ((status = NetValueGet(pPortString, &ipaddr, IPADDR)) == SUCCESS)
    {
        status = NetValueGet(pPortString, &netmask, NETMASK);
    }
...
static int PrintParameter(char *pPortString, char *pParamString)
{
    ...
    else if(0 == strcmp(pParamString, "cable-state"))
    {
        ...
        status = ct_libnet_get_cable_state(pPortString, &state, NULL);
    ...
    else if(0 == strcmp(pParamString, "mac-address"))
    {
        ...
        mac[0] = '\0';
        status = ct_libnet_get_mac_addr_from_hw(pPortString, mac, sizeof(mac), NULL);
    ...
    else if(0 == strcmp(pParamString, "ip-address"))
    {
        ...
        ip[0] = '\0';
        status = ct_libnet_get_ip_addr_from_hw(pPortString, ip, sizeof(ip), IP_FORMAT_MODE_NO_NETMASK, NULL);
    ...
    else if(0 == strcmp(pParamString, "subnet-mask"))
    {
        ...
        netmask[0] = '\0';
        status = ct_libnet_get_netmask_from_hw(pPortString, netmask, sizeof(netmask), NETMASK_FORMAT_MODE_CLASSIC, NULL);
    ...
    {
        status = PrintDefaultGateway(pPortString);
    } else if(0 == strcmp(pParamString, "default-gateway-nofail"))
    {
        PrintDefaultGateway(pPortString);
        status = SUCCESS;
    }
...
static int __ct_getopt(int argc, char **argv, struct options *opts)
{
    ...
    while(1)
    {
        ...
        if(-1 == (c = getopt_long(argc, argv, "h", long_options, &option_index)))
        {
            break;
        }

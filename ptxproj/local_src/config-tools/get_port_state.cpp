...
static void getSnmpState(void)
{
  auto config = wago::snmp_config_lib::ReadSnmpConfig();
...
void checkSnmpState(const ::std::string& expected_value)
{
  auto config = wago::snmp_config_lib::ReadSnmpConfig();
  bool is_enabled = (expected_value == "enabled");

  if(config.snmp_enable_ == is_enabled) {
    printf("checked");
  }
  ...
  else
  {
    ...
    else
    {
      ...
      if(status == SUCCESS)
      {
        ...
        else if(strcmp((const char *)astExeNameAssignTab[parameterIndex].inputString, "snmp") == 0)
        {

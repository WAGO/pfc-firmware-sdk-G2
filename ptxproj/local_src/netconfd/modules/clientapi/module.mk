...
libnetconf_install_headers += $(call fglob_r,$(PROJECT_ROOT)/modules/common/extern/clientapi, h hpp)
...
libnetconf.so_DISABLEDWARNINGS += inline psabi
...
libnetconf.so_LIBS += boost_system boost_filesystem common
libnetconf.so_STATICALLYLINKED += common
...
libnetconf.so_CPPFLAGS += -isystem ${SYSROOT}/usr/include/dbus-1.0
...
libnetconf.so_LDFLAGS += -Wl,--exclude-libs
...
libnetconf.a_DISABLEDWARNINGS += inline psabi
...
libnetconf.a_LIBS += boost_system boost_filesystem common
libnetconf.a_STATICALLYLINKED += common
libnetconf.a_PKG_CONFIGS = dbus-1
libnetconf.a_CPPFLAGS += $(call uniq, $(libnetconf.a_INCLUDES))
libnetconf.a_CPPFLAGS += $(call pkg_config_cppflags,$(libnetconf.a_PKG_CONFIGS))
libnetconf.a_CPPFLAGS += -isystem ${SYSROOT}/usr/include/dbus-1.0
...
libnetconf_tests.elf_DISABLEDWARNINGS += packed inline psabi
...
libnetconf_tests.elf_CLANG_TIDY_RULESET = $(CLANG_TIDY_CHECKS)
libnetconf_tests.elf_CLANG_TIDY_CHECKS += $(GTEST_CLANG_TIDY_CHECKS)
libnetconf_tests.elf_CLANG_TIDY_CHECKS += $(SHARED_CLANG_TIDY_CHECKS)
...
$(libnetconf_header_install_dir)/% : $(PROJECT_ROOT)/modules/common/extern/clientapi/%
	mkdir -p $(dir $@) && cp $(realpath $<) $@

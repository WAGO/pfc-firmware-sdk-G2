...
TEST_F(BridgeConfigTest, AssignInterfaceToBridge) {
  ...
  dut.AssignInterfaceToBridge("ethX1", "br0");
  EXPECT_EQ(R"({"br0":["X1"]})", jc.ToJsonString(dut.GetConfig()));

  dut.AddBridge("br1");
  dut.AssignInterfaceToBridge("ethX1", "br1");

  EXPECT_EQ(R"({"br0":[],"br1":["X1"]})", jc.ToJsonString(dut.GetConfig()));
}

TEST_F(BridgeConfigTest, AssignInterfaceToBridgeAndRemoveEmptyBridge) {
  BridgeConfig dut{};
  dut.AddBridge("br0");
  dut.AddBridge("br1");
  dut.AssignInterfaceToBridge("ethX1", "br1");
  ...
  JsonConverter jc;
  ASSERT_EQ(R"({"br0":[],"br1":["X1"]})", jc.ToJsonString(dut.GetConfig()));

  dut.AssignInterfaceToBridge("ethX1", "br0", true);
  EXPECT_EQ(R"({"br0":["X1"]})", jc.ToJsonString(dut.GetConfig()));
...
TEST_F(BridgeConfigTest, GetBridgeOfInterface) {
  ...
  ::std::string actual_bridge = dut.GetBridgeOfInterface("ethX1");
...
TEST_F(BridgeConfigTest, BridgeConfigToString) {
  BridgeConfig config;
  auto status = MakeBridgeConfig(R"({"br0":["X1","X2"], "br1":["X11"]})", config);
  ::std::string config_str = ToString(config);
  EXPECT_EQ("br0=X1,X2 br1=X11", config_str);

  status = MakeBridgeConfig(R"({"br0":["ethX1","ethX2"], "br1":["ethX11"]})", config);
  config_str = ToString(config);
  EXPECT_EQ("br0=X1,X2 br1=X11", config_str);
}

TEST_F(BridgeConfigTest, BridgeConfigToJson) {
  auto json_str = R"({"br0":["X1"]})";
  BridgeConfig config;
  auto status = MakeBridgeConfig(json_str,config);

  EXPECT_EQ(json_str, ToJson(config));
}

TEST_F(BridgeConfigTest, BridgeConfigToPrettyJson) {
  auto pretty_json_str = R"({
  "br0": [
    "X1"
  ]
})";
  BridgeConfig config;
  auto status = MakeBridgeConfig(pretty_json_str,config);

  EXPECT_EQ(pretty_json_str, ToPrettyJson(config));
}

TEST_F(BridgeConfigTest, CheckEqualityOfBridgeConfigs){
  BridgeConfig config1;
  MakeBridgeConfig(R"({"br0":["X1","X2"], "br1":["X11"]})", config1);
  BridgeConfig config2;
  MakeBridgeConfig(R"({"br0":["X1","X2"], "br1":["X11"]})", config2);
  BridgeConfig config3;
  MakeBridgeConfig(R"({"br0":["X1","X2"]})", config3);

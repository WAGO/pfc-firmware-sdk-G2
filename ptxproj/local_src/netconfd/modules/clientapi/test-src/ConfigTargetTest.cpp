...
class ConfigTest_Target : public testing::Test {
 public:
  ...
  ConfigTest_Target() {
    ...
    seperated_.AssignInterfaceToBridge("ethX1", "br0");
    seperated_.AddBridge("br1");
    seperated_.AssignInterfaceToBridge("ethX2", "br1");

    switched_.AddBridge("br0");
    switched_.AssignInterfaceToBridge("ethX1", "br0");
    switched_.AssignInterfaceToBridge("ethX2", "br0");
...
TEST_F(ConfigTest_Target, DeleteIpConfig) {
  ...
  IPConfigs actual_2;
  GetIPConfigs(actual_2);
  IPConfig expected { bridge1, IPSource::NONE, netconf::ZeroIP, netconf::ZeroNetmask };

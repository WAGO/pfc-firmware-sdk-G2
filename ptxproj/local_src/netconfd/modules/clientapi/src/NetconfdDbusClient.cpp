...
static DbusMsgPtr CreateInterfaceConfigMessage(::std::string method_name)  // NOLINT(performance-unnecessary-value-param)
    {                                                                    // NOLINT(performance-unnecessary-value-param)
  return DbusMsgPtr(dbus_message_new_method_call(dbus_target, "/de/wago/netconfd/interface_config",  // object to call on
                                                 "de.wago.netconfd1.interface_config",  // interface-config to call on
                                                 method_name.c_str()));
}

static DbusMsgPtr CreateInterfaceMessage(::std::string method_name)  // NOLINT(performance-unnecessary-value-param)
    {                                                                    // NOLINT(performance-unnecessary-value-param)
  return DbusMsgPtr(dbus_message_new_method_call(dbus_target, "/de/wago/netconfd/interface",  // object to call on
                                                 "de.wago.netconfd1.interface",  // interface to call on
                                                 method_name.c_str()));
}
...
static ::std::vector<::std::string> GetMessageStrings(const netconf::DbusMsgPtr &replymsg) {
  ...
  for (auto has_argument = dbus_message_iter_init(replymsg.get(), &args); static_cast<bool>(has_argument);
      has_argument = dbus_message_iter_next(&args)) {
...
DbusResult NetconfdDbusClient::GetBridgeConfig() {
  auto msg = CreateInterfaceConfigMessage("get");
  return GetStrings(msg);
}

DbusResult NetconfdDbusClient::SetBridgeConfig(const ::std::string &json_config) {
  auto msg = CreateInterfaceConfigMessage("set");
...
DbusResult NetconfdDbusClient::GetInterfaceConfigs() {
  auto msg = CreateInterfaceConfigMessage("getinterfaceconfig");
  return GetStrings(msg);
}

DbusResult NetconfdDbusClient::GetInterfaceStatuses() {
  auto msg = CreateInterfaceConfigMessage("getinterfacestatuses");
  return GetStrings(msg);
}

DbusResult NetconfdDbusClient::SetInterfaceConfigs(const ::std::string &config) {
  auto msg = CreateInterfaceConfigMessage("setinterfaceconfig");
  return Send(msg, config);
}

DbusResult NetconfdDbusClient::GetDeviceInterfaces() {
  auto msg = CreateInterfaceConfigMessage("getdeviceinterfaces");
  return GetStrings(msg);
}

DbusResult NetconfdDbusClient::AddInterface(const ::std::string &config) {
  auto msg = CreateInterfaceMessage("add");
  return Send(msg, config);
}

DbusResult NetconfdDbusClient::DeleteInterface(const ::std::string &config) {
  auto msg = CreateInterfaceMessage("delete");
  return Send(msg, config);
}
...
DbusResult NetconfdDbusClient::Send(const DbusMsgPtr &msg) {
  ...
  SharedSemaphore dbus_access_semaphore_ { dbus_access_semaphore_name, dbus_access_semaphore_init_value };
  {
...
DbusResult NetconfdDbusClient::GetStrings(const DbusMsgPtr &msg) {
  ...
  return { strings[0], strings.size() >= 2 ? strings[1] : "" };
}

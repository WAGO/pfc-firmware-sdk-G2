...
struct DbusResult {
  DbusResult() = default;
  explicit DbusResult(const ::std::string& error): error_{MakeError(error)}{}
  ...
  DbusResult(::std::string value, const ::std::string& error): value_json_{::std::move(value)}, error_{MakeError(error)} {

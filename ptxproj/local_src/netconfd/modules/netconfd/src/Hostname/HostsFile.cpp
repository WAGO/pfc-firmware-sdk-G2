...
::std::string CreateEntriesSection(::std::vector<Address> ip_addresses, bool needs_localhost_entry,
...
void UpdateHostsFile(::std::vector<Address> &ip_addresses, const ::std::string &hostname, const ::std::string &domain) {
  ...
  if (status.IsOk()) {
    new_hosts = UpdateFileContent(old_hosts, ip_addresses, hostname, domain);
  } else {
    new_hosts = CreateEntriesSection(ip_addresses, true, hostname, domain);
  }
...
::std::string UpdateFileContent(::std::string old_hosts, ::std::vector<Address> &ip_addresses,
  ...
  auto section = CreateEntriesSection(ip_addresses, needs_localhost_entry, hostname, domain);

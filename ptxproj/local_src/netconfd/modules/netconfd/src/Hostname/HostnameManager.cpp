...
LeaseFile GetLeaseFile(const Interface &interface) {
  LeaseFile lease;
  lease.Parse(LeaseFile::GetLeaseFilePath(interface));
  return lease;
}

::std::vector<Address> GetIPsOfIPConfigs(const IPConfigs &ip_configs) {
...
void HostnameManager::OnLeaseFileChange(const Interface &interface) {
  ...
  if (lease_file.Exists()) {
    ...
    LogInfo("use lease file hostname: " + hostname_ + " domain: " + domain_ + " of interface: " + interface.GetName());
    SetKernelHostname(hostname_);
    UpdateEtcHosts(GetCurrentIPConfigs(), hostname_, domain_);
  } else {
...
void HostnameManager::OnLeaseFileRemove(const Interface &interface) {
  /*

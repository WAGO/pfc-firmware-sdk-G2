...
class IPersistenceProvider {
 public:
  ...
  virtual Status Write(const Interfaces& config) = 0;
  virtual void Read(Interfaces& config) = 0;

  virtual Status Read(BridgeConfig& config, IPConfigs& configs, Interfaces& interfaces) = 0;

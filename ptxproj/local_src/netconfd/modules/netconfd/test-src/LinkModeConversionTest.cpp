...
constexpr auto generateLinkModes(Args ...args) {
  ...
  for(auto bit : bits){
    gsl::at(modes,static_cast<int>(bit)/32) |= 1 << (bit % 32);
  }
  return modes;
}
...
TEST(LinkModeConversionTest, ConvertToLinkModeVector) {
  ...
  EXPECT_THAT(linkmodes, testing::ElementsAre(LinkMode{10, Duplex::HALF}, LinkMode{10, Duplex::FULL}, LinkMode{100, Duplex::HALF}, LinkMode{100, Duplex::FULL}, LinkMode{10000, Duplex::FULL}));

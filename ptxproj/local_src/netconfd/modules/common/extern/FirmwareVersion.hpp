...
class FirmwareVersion {
 public:
  ...
  constexpr FirmwareVersion(int major, int minor, int bugfix, int index) noexcept
  :
          major_ { major },
          minor_ { minor },
          bugfix_ {bugfix },
  ...
  bool IsEqualToMajorMinorOf(const FirmwareVersion &other) const {
    return ((major_ == other.major_) and (minor_ == other.minor_));
  }

  bool IsLessThanMajorMinorOf(const FirmwareVersion &other) const {
    if (major_ < other.major_){
      return true;
    }
    if ((major_ == other.major_) and (minor_ < other.minor_)){
  ...
  bool IsGreaterThanMajorMinorOf(const FirmwareVersion &other) const {
    return (not IsEqualToMajorMinorOf(other) and not IsLessThanMajorMinorOf(other));
  }
...
constexpr FirmwareVersion FIRMWARE_15 { 3,  3, 17, 15 };
constexpr FirmwareVersion FIRMWARE_22 { 3, 10,  8, 22 };

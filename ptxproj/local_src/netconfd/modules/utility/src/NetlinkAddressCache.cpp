...
static ChangeType ConvertNetlinkAction(int netlink_act) {
  switch (netlink_act) {
    case NL_ACT_NEW:
...
class NetlinkAddressCache::Impl {
 public:
  ...
  void CallEventHandler(rtnl_addr* rtnl_addr, int change_type) {
    ...
    int index = rtnl_addr_get_ifindex(rtnl_addr);

    auto address = GetIPAddress(rtnl_addr);
    auto netmask = GetNetmask(rtnl_addr);

    event_handler_->OnAddressChange(ConvertNetlinkAction(change_type), index, address,
                                    netmask);
  }
  ...
  rtnl_addr* GetRtnlAddress(int if_index) {
    ...
    auto cb = [] (nl_object* o, void* p) {
      auto addr = reinterpret_cast<rtnl_addr*>(o); // NOLINT: Need reinterpret_cast to cast from nl_object*.
  ...
  Netmask GetNetmask(rtnl_addr* rtnl_addr) {
    int prefix_len = rtnl_addr_get_prefixlen(rtnl_addr);
    return netconf::GetNetmask(static_cast<uint32_t>(prefix_len));
  }

  Netmask GetNetmask(int if_index) {
    auto rtnl_addr = GetRtnlAddress(if_index);
    return rtnl_addr == nullptr ? ZeroNetmask : GetNetmask(rtnl_addr);
  }

  Address GetIPAddress(rtnl_addr* rtnl_addr) {
  ...
  Address GetIPAddress(int if_index) {
    auto rtnl_addr = GetRtnlAddress(if_index);
...
NetlinkAddressCache::NetlinkAddressCache(nl_sock* nl_sock, nl_cache_mngr* nl_cache_mgr) {
  impl_ = ::std::make_unique<NetlinkAddressCache::Impl>(nl_sock, nl_cache_mgr);
}

NetlinkAddressCache::~NetlinkAddressCache() = default;

void NetlinkAddressCache::Resync(nl_sock* nl_sock) {
...
Address NetlinkAddressCache::GetIPAddress(int if_index) {
  return impl_->GetIPAddress(if_index);
}

Netmask NetlinkAddressCache::GetNetmask(int if_index) {

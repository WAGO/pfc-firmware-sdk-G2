...
DBusHandlerRegistry::DBusHandlerRegistry() {
  // interface_config
  interface_config_object_ = netconfd_object_skeleton_new("/de/wago/netconfd/interface_config");
  interface_config_ = netconfd_interface_config_skeleton_new();
  netconfd_object_skeleton_set_interface_config(interface_config_object_, interface_config_);
  ...
  // interfaces
  interface_object_ = netconfd_object_skeleton_new("/de/wago/netconfd/interface");
  interface_ = netconfd_interface_skeleton_new();
  netconfd_object_skeleton_set_interface(interface_object_, interface_);

  GSignalConnect(interface_, "handle-add", AddInterface, this);

  GSignalConnect(interface_, "handle-delete", DeleteInterface, this);
}

DBusHandlerRegistry::~DBusHandlerRegistry() {
  g_object_unref(interface_config_object_);
...
GDBusObjectSkeleton* DBusHandlerRegistry::GetInterfaceConfigObject() const {
  return G_DBUS_OBJECT_SKELETON(interface_config_object_);
}
...
gboolean DBusHandlerRegistry::AddInterface(netconfdInterface *object,
                                           GDBusMethodInvocation *invocation,
                                           const gchar *arg_config, gpointer user_data) {
  auto this_ = reinterpret_cast<DBusHandlerRegistry*>(user_data);

  if (this_->add_interface_handler_) {
    auto result = this_->add_interface_handler_(arg_config);
    netconfd_interface_complete_add(object, invocation, result.c_str());
  } else {
    g_dbus_method_invocation_return_dbus_error(invocation, "de.wago.netconfd1.interface.Error.Add",
                                               "No Handler for AddInterface request");
  }
  return true;
}

gboolean DBusHandlerRegistry::DeleteInterface(netconfdInterface *object,
                                              GDBusMethodInvocation *invocation,
                                              const gchar *arg_config, gpointer user_data) {
  auto this_ = reinterpret_cast<DBusHandlerRegistry*>(user_data);

  if (this_->delete_interface_handler_) {
    auto result = this_->delete_interface_handler_(arg_config);
    netconfd_interface_complete_delete(object, invocation, result.c_str());
  } else {
    g_dbus_method_invocation_return_dbus_error(invocation, "de.wago.netconfd1.interface.Error.Delete",
                                               "No Handler for DeleteInterface request");
  }
  return true;
}

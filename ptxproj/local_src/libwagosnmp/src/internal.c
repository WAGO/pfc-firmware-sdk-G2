...
static int get_netsnmp_privacy_type(enum SnmpPrivacyProtocol privacy) {
  switch (privacy) {
    case PRIVACY_NONE:
      return USM_CREATE_USER_PRIV_NONE;
    case PRIVACY_DES:
      return USM_CREATE_USER_PRIV_DES;
    case PRIVACY_AES:
    case PRIVACY_AES128:
      return USM_CREATE_USER_PRIV_AES;
    case PRIVACY_AES192:
      return USM_CREATE_USER_PRIV_AES192;
    case PRIVACY_AES192C:
      return USM_CREATE_USER_PRIV_AES192_CISCO;
    case PRIVACY_AES256:
      return USM_CREATE_USER_PRIV_AES256;
    case PRIVACY_AES256C:
      return USM_CREATE_USER_PRIV_AES256_CISCO;
    default:
      return USM_CREATE_USER_PRIV_DFLT;
  }
}

static int get_netsnmp_auth_type(enum SnmpAuhtenticationProtocol auth) {
  switch (auth) {
    case AUTH_NONE:
      return NETSNMP_USMAUTH_NOAUTH;
    case AUTH_MD5:
      return USM_CREATE_USER_AUTH_MD5;
    case AUTH_SHA:
      return USM_CREATE_USER_AUTH_SHA;
    case AUTH_SHA224:
      return USM_CREATE_USER_AUTH_SHA224;
    case AUTH_SHA256:
      return USM_CREATE_USER_AUTH_SHA256;
    case AUTH_SHA384:
      return USM_CREATE_USER_AUTH_SHA384;
    case AUTH_SHA512:
      return USM_CREATE_USER_AUTH_SHA512;
    default:
      return USM_CREATE_USER_AUTH_DFLT;
  }
}
...
INTERNAL_SYM tWagoSnmpReturnCode INTERNAL_AddVarAndSend(char sOID[128], netsnmp_variable_list *stData,
  ...
  if (sOID[0] != 0 && !SNMP_TLV_IS_NULL(stData)) {
    ...
    if (NULL == snmp_pdu_add_variable(pdu, OID, OID_length, (u_char)var.type, var.buf, var.len)) {
      return WAGOSNMP_RETURN_ERR_MALLOC;
    }
  }
...
static void snmp_mutex_init(void) {

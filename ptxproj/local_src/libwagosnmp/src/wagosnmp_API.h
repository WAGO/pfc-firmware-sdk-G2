...
enum SnmpAuhtenticationProtocol {
  AUTH_NONE = 0,
  AUTH_MD5,
  AUTH_SHA,
  AUTH_SHA224,
...
typedef struct {
  ...
  size_t szHost;
  char *sOID; /* VAR_INPUT */ /* SNMP unique object identifier(OID) of requested data, e.g. '1.3.6.1.2.1.1.6.0' for
                                 "sysLocation"; also used as enterprise OID in case of trap/inform */
  size_t szOID;
...
tWagoSnmpReturnCode libwagosnmp_SendTrap(char *sEnterprise, tWagoSnmpTrapType trap_type, uint16_t specific_type,
                                         char *sOID, tWagoSnmpTlv *stData);
tWagoSnmpReturnCode libwagosnmp_SendEntTrap(uint16_t specific_type);
tWagoSnmpReturnCode libwagosnmp_SendTrapToAdrV1(char *sHost, char *sCommunity, char *sEnterprise,
                                                tWagoSnmpTrapType trap_type, uint16_t specific_type, char *sOID,
                                                tWagoSnmpTlv *stTlvData);
tWagoSnmpReturnCode libwagosnmp_SendTrapToAdrV2c(char *sHost, char *sCommunity, char *sEnterprise, char *sOID,
                                                 tWagoSnmpTlv *stTlvData);
tWagoSnmpReturnCode libwagosnmp_SendTrapToAdrV3(char *sHost, char *sEngineId, char *sUsername,
                                                tWagoSnmpSecLevel authLevel, tWagoSnmpAuthProt authProt, char *authPass,
                                                tWagoSnmpPrivProt privProt, char *privPass, char *sEnterprise,

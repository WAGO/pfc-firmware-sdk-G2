...
#define INIT_SNMP_AGENT_ONCE \
  INIT_SNMP_ONCE;            \
  (void)pthread_once(&snmp_agent_is_initialized, INTERNAL_InitSnmpAgent);
...
#define CALC_OBJ_SIZE(x, y, z)                         \
  {                                                    \
    if (z) {                                           \
      x = sizeof(tOidObject);                          \
    } else {                                           \
      x = (sizeof(tOidObject) + (y - OID_BUFFER_LEN)); \
    }                                                  \
  }

...
bool Community::operator <(const Community &rhs) const {
  const auto lhs_key = name_ + std::to_string(static_cast<int>(access_));
  const auto rhs_key = rhs.name_  + std::to_string(static_cast<int>(rhs.access_));
...
bool TrapReceiverV1V2c::operator <(const TrapReceiverV1V2c &rhs) const {
  const auto lhs_key = community_ + host_ + std::to_string(static_cast<int>(rhs.version_));
  const auto rhs_key = rhs.community_ + rhs.host_ + std::to_string(static_cast<int>(rhs.version_));
  return lhs_key < rhs_key;
}

TrapReceiverV3::TrapReceiverV3(::std::string name, SecurityLevel security_level,
                               AuthenticationType authentication_type, ::std::string authentication_key,
                               Privacy privacy, ::std::string privacy_key, ::std::string host)
    : name_ { ::std::move(name) },
      security_level_ { security_level },
      authentication_type_ { authentication_type },
      authentication_key_ { ::std::move(authentication_key) },
      privacy_ { privacy },
  ...
  if (privacy != Privacy::None && privacy_key_.empty()) {
    privacy_key_ = authentication_key_;
  }
...
bool TrapReceiverV3::operator ==(const TrapReceiverV3 &rhs) const {
  return (name_ == rhs.name_ && host_ == rhs.host_ && security_level_ == rhs.security_level_
      && authentication_key_ == rhs.authentication_key_ && authentication_type_ == rhs.authentication_type_
      && privacy_ == rhs.privacy_ && privacy_key_ == rhs.privacy_key_);
}

bool TrapReceiverV3::operator <(const TrapReceiverV3 &rhs) const {
  const auto lhs_key = name_ + host_;
  const auto rhs_key = rhs.name_ + rhs.host_;
  return lhs_key < rhs_key;
}
...
User::User(::std::string name, Access access, SecurityLevel security_level, AuthenticationType authentication_type,
           ::std::string authentication_key, Privacy privacy, ::std::string privacy_key)
    : name_ { ::std::move(name) },
      access_ { access },
      security_level_ { security_level },
      authentication_type_ { authentication_type },

...
::std::string AuthenticationTypeToString(AuthenticationType a) {
...
AuthenticationType StringToAuthenticationType(::std::string &a) {
  static ::std::map<::std::string, AuthenticationType> lookup = {
    {"MD5", AuthenticationType::MD5 },
    {"SHA", AuthenticationType::SHA, },
    {"SHA224", AuthenticationType::SHA224},
    {"SHA256", AuthenticationType::SHA256},
    {"SHA384", AuthenticationType::SHA384},
    {"SHA512", AuthenticationType::SHA512}
  };
  return lookup.count(a) > 0 ? lookup.at(a) : AuthenticationType::None;
}
...
::std::string PrivacyToString(Privacy p) {
...
Privacy StringToPrivacy(::std::string &a) {
   static ::std::map<::std::string, Privacy> lookup = {
    { "AES", Privacy::AES},
    { "DES", Privacy::DES},
    { "AES128", Privacy::AES128},
    { "AES192", Privacy::AES192},
    { "AES192C", Privacy::AES192C},
    { "AES256", Privacy::AES256},
    { "AES256C", Privacy::AES256C},
  };  

  return lookup.count(a) > 0 ? lookup.at(a) : Privacy::None;
}
...
::std::string UserParameterToString(const SnmpConfig &s) {
  ...
  for (auto &user : s.user_) {
    ...
    switch (user.security_level_) {
      case SecurityLevel::AuthNoPriv:
        ss << " " << AuthenticationTypeToString(user.authentication_type_) << " " << user.authentication_key_;
        break;
      case SecurityLevel::AuthPriv:
        ss << " " << AuthenticationTypeToString(user.authentication_type_) << " " << user.authentication_key_;
        if (user.privacy_key_.empty()) {
          ss << " " << PrivacyToString(user.privacy_) << " " << user.authentication_key_;
        } else {
...
::std::string TrapV3ParameterToString(const SnmpConfig &s) {
  ...
  for (auto &trap : s.trap_receivers_V3_) {
    ...
    switch (trap.security_level_) {
      case SecurityLevel::AuthNoPriv:
        ss << " -a " << AuthenticationTypeToString(trap.authentication_type_) << " -A "
           << trap.authentication_key_;
        break;
      case SecurityLevel::AuthPriv:
        ss << " -a " << AuthenticationTypeToString(trap.authentication_type_) << " -A "
           << trap.authentication_key_;
        if (trap.privacy_key_.empty()) {
          ss << " -x " << PrivacyToString(trap.privacy_) << " -X " << trap.authentication_key_;
        } else {
...
void ExtractAnyTrapV3Parameters(const ConfigLine &l, SnmpConfig &s) {
  ...
  AuthenticationType auth_type = StringToAuthenticationType(value);
...
void ExtractAnyUserCreateParameters(const ConfigLine &l, SnmpConfig &s) {
  ...
  if (security_level == SecurityLevel::AuthPriv || security_level == SecurityLevel::AuthNoPriv) {
    auth_type = StringToAuthenticationType(values[1]);
    auth_key = values[2];
  }
  ...
  (*it).authentication_type_ = auth_type;
  (*it).authentication_key_ = auth_key;
  (*it).privacy_ = privacy;

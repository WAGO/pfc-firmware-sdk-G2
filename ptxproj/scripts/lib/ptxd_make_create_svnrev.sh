...
ptxd_get_svn_revision() {
	...
	rev=`echo $rev | awk '{print $NF}'`
	printf -- 'svn%s%s' "$rev" "$suffix"
    elif commitID=`git rev-parse --short HEAD 2>/dev/null`; then
        printf -- 'git%s%s' "$commitID" "$suffix"
    else
	ptxd_bailout "Not a svn or git repository! Can't create '${filepath}'"
    fi
}
...
ptxd_get_svn_build_url() {
	...
	build_url=`echo $build_url | awk '{print $NF}'`
	printf -- '%s' "$build_url"
    elif build_url=`git config --get remote.origin.url 2>/dev/null`; then
	printf -- '%s' "$build_url"
    else
	ptxd_bailout "Not a svn or git repository! Can't create '${filepath}'"
    fi

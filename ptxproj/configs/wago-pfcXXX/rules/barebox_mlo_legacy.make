...
BAREBOX_MLO_LEGACY_SUFFIX	:= tar.bz2
BAREBOX_MLO_LEGACY		:= barebox-$(BAREBOX_MLO_LEGACY_VERSION)
BAREBOX_MLO_LEGACY_URL		:= http://www.barebox.org/download/$(BAREBOX_MLO_LEGACY).$(BAREBOX_MLO_LEGACY_SUFFIX)
BAREBOX_MLO_LEGACY_DIR		:= $(BUILDDIR)/barebox_mlo_legacy-$(BAREBOX_MLO_LEGACY_VERSION)
BAREBOX_MLO_LEGACY_SOURCE	:= $(SRCDIR)/$(BAREBOX_MLO_LEGACY).$(BAREBOX_MLO_LEGACY_SUFFIX)
...
$(STATEDIR)/barebox_mlo_legacy.targetinstall:
	...
	if [ ! -e "$(IMAGEDIR)/MLO_LEGACY" ]; then \
		ptxd_get_path "$(BAREBOX_MLO_LEGACY_DIR)/MLO" \
			"$(BAREBOX_MLO_LEGACY_DIR)/barebox.bin.ift" && \
		install -D -m644 "$${ptxd_reply}" "$(IMAGEDIR)/MLO_LEGACY"; \
	else \
		rm -f $(IMAGEDIR)/MLO_LEGACY; \
	fi

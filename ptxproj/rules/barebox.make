...
BAREBOX_VERSION	:= $(call remove_quotes, \
		$(PTXCONF_BAREBOX_VERSION)-$(PTXCONF_BAREBOX_LOCALVERSION))
BAREBOX_SUFFIX	:= tgz
BAREBOX_MD5	:= $(call remove_quotes,$(PTXCONF_BAREBOX_MD5))
BAREBOX_MD5_FILE := src/barebox-$(BAREBOX_VERSION).$(BAREBOX_SUFFIX).md5
BAREBOX		:= barebox-$(BAREBOX_VERSION)
BAREBOX_URL	:= $(call jfrog_template_to_url, BAREBOX)
BAREBOX_SOURCE	:= $(SRCDIR)/barebox-$(BAREBOX_VERSION).$(BAREBOX_SUFFIX)
BAREBOX_DIR	:= $(BUILDDIR)/$(BAREBOX)
BAREBOX_LICENSE	:= GPL-2.0
BAREBOX_DEVPKG	:= NO

BAREBOX_CONFIG	:= $(call remove_quotes, \
		$(PTXDIST_PLATFORMCONFIGDIR)/$(PTXCONF_BAREBOX_CONFIG))
...
$(BAREBOX_SOURCE):
	...
	$(call ptx/in-path, PTXDIST_PATH, scripts/wago/artifactory.sh) fetch \
		'$(BAREBOX_URL)' src/barebox-$(BAREBOX_VERSION).$(BAREBOX_SUFFIX) '$(BAREBOX_MD5_FILE)'
	@$(call touch)
# ----------------------------------------------------------------------------
# Prepare
# ----------------------------------------------------------------------------
